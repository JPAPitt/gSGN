...
for expi in range(expn):
    ...
    ex = 10.0
    dx = ((ex - sx))/ncurr
    xn = arange(sx-0.1*dx ,ex+ dx,dx)
    nxn = len(xn)
    a1 = 0.5
    
    h,u,x,hA,uA = initialhu(xn,dx,a1)
    ...
    L2 = norm(u - uc ,ord=2)/norm(u ,ord=2)
    
    #Just jph
    # L2ph = norm(u[3:-3:4] - ucN[3:-3:4]  ,ord=2)/norm(u[3:-3:4]  ,ord=2)
