...
from matplotlib.pyplot import plot,loglog,title,xlabel,ylabel,legend
...
def Reconjmh(qA,j,theta,dx):
    ...
    na = minmod(P2a0,P2a1,P2a2)
    nb = minmod(P2b0 - P1lima,P2b1 - P1lima,P2b2 - P1lima ) +P1lima
    nc = minmod(P2c0 - qA[j],P2c1 - qA[j],P2c2 - qA[j]) + qA[j]
...
x3 = arange(sx ,ex+(0.1)*dx,dx)
nx = len(x3)

hA,GA = initialhDB(x3,dx,2,1)


dt = 0.5*dx/sqrt(2*g)


hAN3,GAN3 = Solver(hA,GA,0,4,g,dx,dt,nGcells)
