...
def InitialSolitonRep(x,dx,t,ga,a0,a1,b1):
    n = len(x)
    hp = []
    Gp = []
    up = []
...
def FEM(hR,GR,uR,u0,du0,u1,du1,beta1,n,dx,nG):
    ...
    #    #elementwisematrices
        GMatE,AMatE = FEMElem(hR,GR,beta1,dx,i-1)
...
for ij in range(5,6):
    ...
    NuC,nxu  = AverageEdges(Nu,xn,dx)
    NGC,nxG = AverageEdges(NG,xn,dx)
    ...
    L2u = norm(NuC[::2] - NuN[::2],ord=2)/norm(NuC[::2],ord=2)
    L2du = norm(NuC[1::2] - NuN[1::2],ord=2)/norm(NuC[1::2],ord=2)
