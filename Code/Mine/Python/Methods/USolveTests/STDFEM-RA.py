...
def PointsFromPolyFromPoints4(a0,a1,a2,a3,a4,x):
...
def ComplexDerivTerm(q,xn,dx):
    ...
    for i in range(n):
        ...
        a4 = 1701*hjmh**2/(4*dx**6) - 5103*hjmh*hjms/(2*dx**6) - 1701*hjmh*hjph/(2*dx**6) + 5103*hjmh*hjps/(2*dx**6) + 15309*hjms**2/(4*dx**6) + 5103*hjms*hjph/(2*dx**6) - 15309*hjms*hjps/(2*dx**6) + 1701*hjph**2/(4*dx**6) - 5103*hjph*hjps/(2*dx**6) + 15309*hjps**2/(4*dx**6)
        a3 = -567*hjmh**2/(2*dx**5) + 1134*hjmh*hjms/dx**5 - 567*hjmh*hjps/dx**5 - 1701*hjms**2/(2*dx**5) + 567*hjms*hjph/dx**5 + 567*hjph**2/(2*dx**5) - 1134*hjph*hjps/dx**5 + 1701*hjps**2/(2*dx**5)
        a2 = 243*hjmh**2/(8*dx**4) + 891*hjmh*hjms/(4*dx**4) + 405*hjmh*hjph/(4*dx**4) - 1539*hjmh*hjps/(4*dx**4) - 6237*hjms**2/(8*dx**4) - 1539*hjms*hjph/(4*dx**4) + 6885*hjms*hjps/(4*dx**4) + 243*hjph**2/(8*dx**4) + 891*hjph*hjps/(4*dx**4) - 6237*hjps**2/(8*dx**4)
        a1 = 45*hjmh**2/(8*dx**3) - 207*hjmh*hjms/(2*dx**3) + 153*hjmh*hjps/(4*dx**3) + 1215*hjms**2/(8*dx**3) - 153*hjms*hjph/(4*dx**3) - 45*hjph**2/(8*dx**3) + 207*hjph*hjps/(2*dx**3) - 1215*hjps**2/(8*dx**3)
        a0 = -35*hjmh**2/(64*dx**2) + 153*hjmh*hjms/(32*dx**2) - 37*hjmh*hjph/(32*dx**2) + 207*hjmh*hjps/(32*dx**2) + 405*hjms**2/(64*dx**2) + 207*hjms*hjph/(32*dx**2) - 1053*hjms*hjps/(32*dx**2) - 35*hjph**2/(64*dx**2) + 153*hjph*hjps/(32*dx**2) + 405*hjps**2/(64*dx**2)
        
        
        Termmh = PointsFromPolyFromPoints4(a0,a1,a2,a3,a4,-dx/2)
        Termms = PointsFromPolyFromPoints4(a0,a1,a2,a3,a4,-dx/6)
        Termps = PointsFromPolyFromPoints4(a0,a1,a2,a3,a4,dx/6)
        Termph = PointsFromPolyFromPoints4(a0,a1,a2,a3,a4,dx/2)
        # a3,a2,a1,a0 = PolyFromPoints(qjmh,qjms,qjps,qjph,dx)
...
for ij in range(10):
    ...
    hTerm = ComplexDerivTerm(hN,xn,dx)
    hTermA = ComplexDerivTerm(h,xn,dx)
    ...
    L2b2Term = norm(b1/4*ga*hN**2*hTerm - b1/4*ga*h**2*hTermA ,ord=2)  / norm(b1/4*ga*h**2*hTermA ,ord=2) 
