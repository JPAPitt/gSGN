...
def LeftLocLogic(qA,j,dx):
    #For a given j, choose the right stencil
    #In eno you choose based on expanding stencil, comparing the highest order term of interpolating polynomial
    #Only want LeftLoc values in [-4,1] range so either we get stenci [j-4,..,j-1] or we get [j+1,...j+4], with all other stencils including j
    LenSten = 4
    LeftLocBigGradsMinusLittleGrads = [0]*4
    for i in range(LenSten):
        BigGrad =  (qA[j+ i - 1 ]  - qA[j - LenSten + i ]) / 3*dx
        LittleGrad1 = (qA[j+ i - 1 ]  - qA[j+ i - 2 ]) / dx
        LittleGrad2 = (qA[j+ i - 2 ]  - qA[j+ i - 3 ]) / dx
        LittleGrad3 = (qA[j+ i - 3 ]  - qA[j+ i - 4 ]) / dx
        
        ListDiff = [ abs(BigGrad - LittleGrad1) , abs(BigGrad - LittleGrad2), abs(BigGrad - LittleGrad3)   ]
        print(ListDiff )
        
        LeftLocBigGradsMinusLittleGrads[i] = mean(ListDiff)
     
    print(LeftLocBigGradsMinusLittleGrads)
    LeftVal = LeftLocBigGradsMinusLittleGrads.index(min(LeftLocBigGradsMinusLittleGrads))
