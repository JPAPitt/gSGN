...
def FEMforG(xn, hp,up,dx):
    ...
    for i in range(1,n-1):
        ...
        un[3*i : 3*i + 3] = up[4*i:4*i+ 3]


    # i = 0
    i=0
    A[3*i : 3*i + 4, 3*i : 3*i + 4] =  eye(4,4)
    b[3*i : 3*i + 4, 3*i : 3*i + 4] =  array(hp[0:4])*array(up[0:4])*eye(4,4)
    un[3*i : 3*i + 4]= up[4*i:4*i+ 4]

    
    # i = 0
    i=n-1
    A[3*i : 3*i + 4, 3*i : 3*i + 4] =  eye(4,4)
    b[3*i : 3*i + 4, 3*i : 3*i + 4] =  array(hp[-4:])*array(up[-4:])*eye(4,4)
    un[3*i: 3*i + 4] = up[4*i:4*i+ 4]
...
def initialhu(x,dx,a0,a1,a2,a3,a4):
    n = len(x)
    hp = []
    up = []
    Gp =[]
    ...
    for i in range(n):
        ...
        #h
        hjmh = a0 + a1*sin(a2*xjmh)
        hjms = a0 + a1*sin(a2*xjms)
        hjps = a0 + a1*sin(a2*xjps)
        hjph = a0 + a1*sin(a2*xjph)
...
h,u,G,x = initialhu(xn,dx,a0,a1,a2,a3,a4)

Gc = FEMforG(xn,h,u,dx)
