...
def CellAverageToCubic(qA,dx):
    ...
    for j in range(1,n-2):
        pa = (3*qA[j] - qA[j-1] - 2* qA[j+1] + qA[j+2]) / (6*dx**3)
        pb = (-2*qA[j] + qA[j-1] +qA[j+1]) / (2*dx**2)
        pc = (-15*qA[j] - 7*qA[j-1] +  27*qA[j+1] -5*qA[j+2]) / (24*dx)
        pd  = (26*qA[j] - qA[j-1] - qA[j+1]) / (24)
        
        qcubic[4*j] = pa*(-dx/2)**3 + pb*(-dx/2)**2 + pc*(-dx/2) + pd
        qcubic[4*j + 1] = pa*(-dx/6)**3 + pb*(-dx/6)**2 + pc*(-dx/6) + pd
        qcubic[4*j + 2] = pa*(dx/6)**3 + pb*(dx/6)**2 + pc*(dx/6) + pd
        qcubic[4*j + 3] = pa*(dx/2)**3 + pb*(dx/2)**2 + pc*(dx/2) + pd
