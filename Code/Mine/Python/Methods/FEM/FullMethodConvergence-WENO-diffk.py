...
def initialhu(x,dx,a1):
    ...
    for i in range(n):
        ...
        #h
        hjmh = 1 + a1*exp(-sqrt(3)*abs(xjmh)**2)
        hjms = 1 + a1*exp(-sqrt(3)*abs(xjms)**2)
        hjps = 1 + a1*exp(-sqrt(3)*abs(xjps)**2)
        hjph = 1 + a1*exp(-sqrt(3)*abs(xjph)**2)
...
def CellAverageToCubic(qA,x,dx):
    ...
    for j in range(3,n-3):
        ...
        pjm1ja = (qA[j] - qA[j-1])/dx
        pjm1jb = qA[j]
        
        pjjp1a = (qA[j+1] - qA[j])/dx
        pjjp1b = qA[j]
        ...
        iw1 = (0.15 / (eps + Bjm1j  )**2)
        iw2 = (0.35 / (eps + Bjjp1 )**2)
        ...
        #    print(w1,w2,w3,w4)
        qa = w1*pjm1ja + w2*pjjp1a
        qb = w1*pjm1jb + w2*pjjp1b
...
for expi in range(expn):
    ...
    ucN = CellAverageToCubic(uA,x,dx)
