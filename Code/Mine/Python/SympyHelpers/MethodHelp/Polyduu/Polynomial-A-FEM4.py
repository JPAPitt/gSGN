...
#Pointwise
GeqPoly = Geq.subs(u,PuPoly).subs(h,PhPoly)
GeqPoly = GeqPoly.doit()

#jmh
GeqPolyjmh = ujmh*hjmh - b1/2*(hjmh**3*diff(PuPoly,(x,2)).subs(x,-dx/2) + 3*hjmh**2*dhjmh*dujmh  )
# GeqPolyjmh = GeqPoly.subs(x,-dx/2)
GeqPolyjmh = GeqPolyjmh.simplify()
GeqPolyjmh = GeqPolyjmh.expand()
GeqPolyjmh = collect(GeqPolyjmh,ujmh)
GeqPolyjmh= collect(GeqPolyjmh,ujph)
GeqPolyjmh = collect(GeqPolyjmh,dujmh)
GeqPolyjmh = collect(GeqPolyjmh,dujph)

CGjmhujph = GeqPolyjmh.subs(ujmh,0).subs(dujmh,0).subs(dujph,0).subs(ujph,1)
CGjmhujmh = GeqPolyjmh.subs(ujmh,1).subs(dujmh,0).subs(dujph,0).subs(ujph,0)
CGjmhdujph = GeqPolyjmh.subs(ujmh,0).subs(dujmh,0).subs(dujph,1).subs(ujph,0)
CGjmhdujmh = GeqPolyjmh.subs(ujmh,0).subs(dujmh,1).subs(dujph,0).subs(ujph,0)


# #j
# GeqPolyj = GeqPoly.subs(x,0)
# GeqPolyj = GeqPolyj.simplify()
# GeqPolyj = GeqPolyj.expand()
# GeqPolyj = collect(GeqPolyj,ujmh)
# GeqPolyj= collect(GeqPolyj,ujph)
# GeqPolyj = collect(GeqPolyj,dujmh)
# GeqPolyj = collect(GeqPolyj,dujph)

# CGjujph = GeqPolyj.subs(ujmh,0).subs(dujmh,0).subs(dujph,0).subs(ujph,1)
# CGjujmh = GeqPolyj.subs(ujmh,1).subs(dujmh,0).subs(dujph,0).subs(ujph,0)
# CGjdujph = GeqPolyj.subs(ujmh,0).subs(dujmh,0).subs(dujph,1).subs(ujph,0)
# CGjdujmh = GeqPolyj.subs(ujmh,0).subs(dujmh,1).subs(dujph,0).subs(ujph,0)


#j+1/2
GeqPolyjph = ujph*hjph - b1/2*(hjph**3*diff(PuPoly,(x,2)).subs(x,dx/2) + 3*hjph**2*dhjph*dujph  )
# GeqPolyjph = GeqPoly.subs(x,dx/2)
GeqPolyjph = GeqPolyjph.simplify()
GeqPolyjph = GeqPolyjph.expand()
GeqPolyjph = collect(GeqPolyjph,ujmh)
GeqPolyjph= collect(GeqPolyjph,ujph)
GeqPolyjph = collect(GeqPolyjph,dujmh)
GeqPolyjph = collect(GeqPolyjph,dujph)

CGjphujph = GeqPolyjph.subs(ujmh,0).subs(dujmh,0).subs(dujph,0).subs(ujph,1)
CGjphujmh = GeqPolyjph.subs(ujmh,1).subs(dujmh,0).subs(dujph,0).subs(ujph,0)
CGjphdujph = GeqPolyjph.subs(ujmh,0).subs(dujmh,0).subs(dujph,1).subs(ujph,0)
CGjphdujmh = GeqPolyjph.subs(ujmh,0).subs(dujmh,1).subs(dujph,0).subs(ujph,0)
