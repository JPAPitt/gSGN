...
Geq = u*Shj - b1*diff(Shj**3*diff(u,x), x)

#Simpsons 3/8 rule to integrate uh
GeqInt = dx/8*(Shj*ujmh + 3*Shj*ujms + 3*Shj*ujps + Shj*ujph) - (b1*Shj**3*dujph - b1*Shj**3*dujmh)
GeqInt =GeqInt.expand()
Geq =Geq.doit()

Geqjmh = Geq.subs(diff(u,(x,2)),ddujmh ).subs(diff(u,(x)),dujmh ).subs(u,ujmh ).subs(diff(h,(x)),dhjmh ).subs(h,Shj )
Geqjmh =Geqjmh.expand()

Geqjph = Geq.subs(diff(u,(x,2)),ddujph ).subs(diff(u,(x)),dujph ).subs(u,ujph ).subs(diff(h,(x)),dhjph ).subs(h,Shj )
Geqjph =Geqjph.expand()

Geqj = Geq.subs(diff(u,(x,2)),dduj ).subs(diff(u,(x)),duj ).subs(u,uj ).subs(diff(h,(x)),dhj ).subs(h,Shj )
Geqj =Geqj.expand()
...
# #Most Expensive step
uPoints = solve([Eq1,Eq2,Eq3, Eq4], [ujph,ujps,ujms,ujmh])
