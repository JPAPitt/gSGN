...
# symbols
x,t,ga,eps = symbols('x t ga eps',real=True)
...
fluxG = u*G + (ga*h**2)/2 - (eps*h**2)*(2*h*diff(u,x)**2 + ga*h*diff(h,(x,2)) + (ga/2)*diff(h,x)**2 )
dfluxGdx = diff(fluxG ,x)


    
#Forced solutions
a0,a1,a2,a3,a4,eps = symbols('a0,a1,a2,a3,a4,eps', positive = True, nonzero = True)
