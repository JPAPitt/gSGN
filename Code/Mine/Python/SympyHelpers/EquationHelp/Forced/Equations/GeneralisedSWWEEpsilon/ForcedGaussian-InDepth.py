...
def SympyToCode(Expr,u,h,eps,xdepth):
    NewExpr = Expr
    ...
    for i in range(xdepth,-1,-1):
        if i == 0 :
            ...
            codestre = 'd' + str(i) + 'edx' + str(i)
        NewExpr= NewExpr.subs(diff(u,(x,i)),codestru ).subs(diff(h,(x,i)),codestrh ).subs(diff(eps,(x,i)),codestre )
    return NewExpr
...
#Source generated by letting epsilon be a function, then expanding G through conservative reg Serre
#dSourceG = diff(eps,t)*diff(h**3* diff(u,x),x) 
dSourceG = diff(eps,t)*diff(h**3* diff(u,x),x) + diff(eps,x)*(3*h**2*u*diff(h,x)*diff(u,x) + h**3*u*diff(u,(x,2)) + 2*h**3*diff(u,x)**2)
...
omega =x - a6*t
cosomega = cos(a7*omega) 
sinomega = sin(-a7*omega)
eforce = a5*cosomega
dedx = simplify(diff(eforce,x).subs(cosomega,'COSOMEGA').subs(sinomega,'SINOMEGA')).subs(omega,'OMEGA') 
dedt = simplify(diff(eforce,t).subs(cosomega,'COSOMEGA').subs(sinomega,'SINOMEGA')).subs(omega,'OMEGA') 

#Condensed Form For Coding
Codeh = hforce.subs(exp1,'EXPPHI1').subs(phi,'PHI')
Codeu = uforce.subs(exp1,'EXPPHI1').subs(phi,'PHI')
Codee = eforce.subs(cosomega,'COSOMEGA').subs(sinomega,'SINOMEGA').subs(omega,'OMEGA') 
CodeG = SympyToCode(G,u,h,eps,3)
...
print()
CodedGdt = dGdt.subs(h,hforce).subs(u,uforce).subs(eps,eforce).doit()
CodedGdt = simplify(CodedGdt.subs(uforce,'ui').subs(hforce,'hi').subs(eforce,'ei') \
                    .subs(exp1,'EXPPHI1').subs(phi,'PHI').subs(dedt,'dedt')) \
                    .subs(a2*t - x,'(-PHI)')
...
Codedfluxhdx = SympyToCode(dfluxhdx,u,h,eps,3)
print('dfluxhdx = ' + str(Codedfluxhdx))
print()

CodedfluxGdx = SympyToCode(dfluxGdx,u,h,eps,3)
...
CodedSourceG = dSourceG.subs(h,hforce).subs(u,uforce).subs(eps,eforce).doit()
CodedSourceG = simplify(CodedSourceG.subs(dedt,'dedt').subs(dedx,0).subs(uforce,'ui').subs(hforce,'hi').subs(eforce,'ei') \
                        .subs(exp1,'EXPPHI1').subs(phi,'PHI')).subs(a2*t - x,'(-PHI)')
CodedSourceG = CodedSourceG.expand()
print('Sa = ' + str(CodedSourceG))
