...
# symbols
x,t,g,eps,alp = symbols('x t g \epsilon \\alpha',real=True)
...
duhdt = diff(u*h,t)
Serre =(diff(u,x)**2 - diff(diff(u,x),t) -u*diff(u,(x,2))) 
Reg = (h*diff(h,(x,2)) + (diff(h,x)**2)/2) 
dfluxuhdx = diff(u**2 *h  + g*h**2/2 + eps*h**3*Serre  - g*eps*alp*h**2*Reg,x)
