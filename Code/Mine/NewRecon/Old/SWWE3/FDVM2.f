...
c ====
      subroutine EvolveStepWrap(xbc_len,hbc,uhbc,ubc,ga
     . ,n_GhstCells,dx,dt)
          
      integer n_GhstCells,xbc_len
      DOUBLE PRECISION hbc(xbc_len),uhbc(xbc_len),ubc(xbc_len)
      DOUBLE PRECISION ga,dx,dt
     ...
     . n_GhstCells,dt,dx,hppbc,uhppbc)
...
c ====
     ...
     . n_GhstCells,dt,dx,hpbc,uhpbc)
      ...
      integer n_GhstCells,xbc_len
      DOUBLE PRECISION dt,dx,ga
      DOUBLE PRECISION hbc(xbc_len),uhbc(xbc_len),ubc(xbc_len),
     . hpbc(xbc_len),uhpbc(xbc_len)
     
     
      DOUBLE PRECISION fih,fiuh,foh,fouh,nAhi,nBhi,nAuhi,
     . nBuhi
     ...
     .      uhbc(xbc_len - n_GhstCells + i)
      ...
      call ReconLinLimGrad(hbc(i-1),hbc(i),hbc(i+1),dx,nAhi,nBhi)
      call ReconLinLimGrad(uhbc(i-1),uhbc(i),uhbc(i+1),dx,nAuhi,nBuhi)
      
      !calculates foh,foG which is flux across x_{i +1/2}
      ! it also updates cdhi,cdGi to be gradient across cell i + 1     
      call Fluxxiph(xbc_len,hbc,uhbc,ubc,ga,dx,
     . i,foh,fouh,nAhi,nBhi,nAuhi,nBuhi)
...
c across cell i +1 (which will be cell i as the method moves to the next cell)    
      subroutine Fluxxiph(xbc_len,hbc,uhbc,ubc,ga,dx,
     . i,foh,fouh,nAhi,nBhi,nAuhi,nBuhi)
     
       integer i,xbc_len
       DOUBLE PRECISION hbc(xbc_len),uhbc(xbc_len),ubc(xbc_len)
       DOUBLE PRECISION ga,dx,
     . foh,fouh,
     . nAhi,nBhi,nAuhi,nBuhi,
     . nAhip1,nBhip1,nAuhip1,nBuhip1
...
c     centered approximation
      ...
      hir = nAhi*(dx/2) + nBhi
      uhir = nAuhi*(dx/2) + nBuhi
      uir = uhir /hir

      !reconstruct values on right side of edge x_{i+1/2}
      call ReconLinLimGrad(hbc(i),hbc(i+1),hbc(i+2),dx,nAhip1,nBhip1)
      call ReconLinLimGrad(uhbc(i),uhbc(i+1),uhbc(i+2),dx,nAuhip1,
     . nBuhip1)
      
      hip1l = nAhip1*(-dx/2) + nBhip1
      uhip1l = nAuhip1*(-dx/2) + nBuhip1
      uip1l = uhip1l/hip1l
...
c Gradients
c ==
      subroutine P1jtojp1A(qaj,qajp1,dx,A,B)
      DOUBLE PRECISION qaj,qajp1,dx,A,B
      A = (qajp1 - qaj)/ dx
      B = qaj
      end

      subroutine P1jm1tojA(qajm1,qaj,dx,A,B)
...
c ====
      subroutine ReconLinLimGrad(qajm1,qaj,qajp1,dx,nA,nB)
      DOUBLE PRECISION qajm1,qaj,qajp1,nA,nB,dx,
     . P1A0,P1A1,P1B0,P1B1
      
      call P1jtojp1A(qaj,qajp1,dx,P1A0,P1B0)
      call P1jm1tojA(qajm1,qaj,dx,P1A1,P1B1)
      
      call minmod(P1B1 - qaj, P1B0 - qaj,P1B1 - qaj,nB)
      nB = nB + qaj
      call minmod(P1A0,P1A1,P1A0,nA)
