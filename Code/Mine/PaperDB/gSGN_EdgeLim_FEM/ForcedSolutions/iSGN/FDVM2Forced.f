...
c=====================================
     ...
     . ucubbc_fin(cubbc_len), habc(xbc_len),Gabc(xbc_len)
      ...
      call FEMforU(hcubbc_fin,Gcubbc_fin,ucubbc_fin,xbc,
     . currenttime,a5,b1a6,b1a7, dx,xbc_len,
     ...
     . ga,currenttime,a0,a1,a2,a3,a4,a5,b1a6,b1a7,b2a6,b2a7,xbc,tolcalc,
     . n_GhstCells,dx,dt,hcubbc_fin,Gcubbc_fin,ucubbc_fin)
...
c Helper Functions - Polynomial Coeffs
      ...
      subroutine minmod(a,b,c,d,e)
      implicit none
      DOUBLE PRECISION a,b,c, d,e,omin,omax
      omin = min(a,b,c,d)
      omax = max(a,b,c,d)
...
c      print *, 'MM O',e,omin,omax
      ...
      subroutine P1WAB(qajm1,qaj,qajp1,dx,A,B)
      implicit none
      DOUBLE PRECISION qaj,qajm1,qajp1,dx,A,B
      A = (qajp1 - qajm1)/ (2*dx)
      B = qaj
      end
      ...
      subroutine P3jm1tojp2ABCD(qajm1,qaj,qajp1,qajp2,dx,A,B,C,D)
      implicit none
      DOUBLE PRECISION qajm1,qaj,qajp1,qajp2,dx,A,B,C,D
      ...
      subroutine P3jm2tojp1ABCD(qajm2,qajm1,qaj,qajp1,dx,A,B,C,D)
      implicit none
      DOUBLE PRECISION qajm2,qajm1,qaj,qajp1,dx,A,B,C,D
...
c Cell Average To Cubic
     ...
     . p3jm2tojp1a3,p3jm2tojp1a2,p3jm2tojp1a1,p3jm2tojp1a0, 
     . p3jm1tojp2a3,p3jm1tojp2a2,p3jm1tojp2a1,p3jm1tojp2a0, 
     ...
     .    p3jtojp3a2,p3jtojp3a1,p3jtojp3a0)  
         ...
         call P3jm1tojp2ABCD(cellavgbc_q(i-1),cellavgbc_q(i),
     .    cellavgbc_q(i+1),cellavgbc_q(i+2),dx,p3jm1tojp2a3,
     .    p3jm1tojp2a2,p3jm1tojp2a1,p3jm1tojp2a0)  

         call P3jm2tojp1ABCD(cellavgbc_q(i-2),cellavgbc_q(i-1),
     .    cellavgbc_q(i),cellavgbc_q(i+1),dx,p3jm2tojp1a3,
     .    p3jm2tojp1a2,p3jm2tojp1a1,p3jm2tojp1a0)           
     ...
     .    p1jm1toja1,p1jm1toja0)  
         call P1WAB(cellavgbc_q(i-1),cellavgbc_q(i),cellavgbc_q(i+1),
     .    dx,p1Wa1,p1Wa0)

     
         call minmod(p1Wa1,p1jm1toja1,p1jtojp1a1,p1jtojp1a1,P1a11w1)
         call minmod(p2jtojp2a2,p2jm1tojp1a2,p2jm2toja2,p2jm2toja2,
     .    P2a22w2)
         call minmod(p3jtojp3a3,p3jm1tojp2a3,p3jm2tojp1a3,p3jm3toja3,
     .    P3a33w3)

         if (dabs(p3jm2tojp1a3) > 10.0**(-15)) then
             w3 = P3a33w3  / p3jm2tojp1a3
         else
            w3 = 0
         end if

         P3a32w3Corr = w3*(p3jm2tojp1a2 - P2a22w2) + P2a22w2

         if (dabs(p2jm1tojp1a2) > 10.0**(-15)) then
            w2 = P2a22w2  / p2jm1tojp1a2
         else
            w2 = 0
         end if         
         
         P2a21w2Corr = w2*(p2jm1tojp1a1 - P1a11w1) + P1a11w1
         
         P3a31w3Corr = w3*(p3jm2tojp1a1 - P2a21w2Corr) + P2a21w2Corr
         ...
         R3a = 0.5*(p3jm1tojp2a3 + p3jm2tojp1a3)
         R3b = 0.5*(p3jm1tojp2a2 + p3jm2tojp1a2)
         R3c = 0.5*(p3jm1tojp2a1 + p3jm2tojp1a1)
         R3d = 0.5*(p3jm1tojp2a0 + p3jm2tojp1a0)
...
c ********************************************************************************
      ...
      subroutine FEMforU(hcub_bc,Gcub_bc,uNcub_bc,xbc,t,a5,b1a6,b1a7,
     . dx,xbc_len,cubbc_len,ncubbc_len,nGhstCell)
      
      !KL = 3
      !KU = 3
      INTEGER cubbc_len,ncubbc_len,nGhstCell,xbc_len
      DOUBLE PRECISION hcub_bc(cubbc_len),uNcub_bc(cubbc_len),
     . Gcub_bc(cubbc_len),xbc(xbc_len)
      ...
      DOUBLE PRECISION b1a6,b1a7,beta1,t,a5
     ...
     . + 235467*hjps**3/366080)   
      ...
      subroutine ForcedSolution(xbc,xbc_len,dx,t,a0,a1,a2,a3,a4,a5,
     . b1a6,b1a7,hcub_bc,Gcub_bc,ucub_bc,cubbc_len)
      implicit none
      
      INTEGER xbc_len,cubbc_len
      DOUBLE PRECISION t,a0,a1,a2,a3,a4,a5,b1a6,b1a7,dx
      DOUBLE PRECISION hcub_bc(cubbc_len),Gcub_bc(cubbc_len),
     . ucub_bc(cubbc_len),xbc(xbc_len)
      ...
      do i=1,xbc_len 
         ...
         PHI  = xbc(i) - 0.5*dx - a2*t
         EXPPHI1 = dexp(-PHI**2 / (2*a3))
         
         hcub_bc(4*i - 3)  = EXPPHI1*a1 + a0
         ucub_bc(4*i - 3)  = EXPPHI1*a4
     ...
     .    -beta1/2d0*(d2udx2*hcub_bc(4*i - 3)**3 
     .    + 3*dhdx*dudx*hcub_bc(4*i - 3)**2)


         ! x_j-1/6
         beta1 = b1a6*(xbc(i) - dx/6.0 - a5*t) + b1a7
         PHI  = xbc(i) - dx/6.0 - a2*t
         EXPPHI1 = dexp(-PHI**2 / (2*a3))
         
         hcub_bc(4*i - 2)  = EXPPHI1*a1 + a0
         ucub_bc(4*i - 2)  = EXPPHI1*a4
         dhdx = -EXPPHI1*PHI*a1/a3
         dudx = -EXPPHI1*PHI*a4/a3
         d2udx2 = -a4*(-PHI**2 + a3)*dexp(-PHI**2/(2*a3))/a3**2
                        
         Gcub_bc(4*i - 2) = ucub_bc(4*i - 2)*hcub_bc(4*i - 2) 
     .    -beta1/2d0*(d2udx2*hcub_bc(4*i - 2)**3 
     .    + 3*dhdx*dudx*hcub_bc(4*i - 2)**2)
         ...
         hcub_bc(4*i - 1)  = EXPPHI1*a1 + a0
         ucub_bc(4*i - 1)  = EXPPHI1*a4
         dhdx = -EXPPHI1*PHI*a1/a3
         dudx = -EXPPHI1*PHI*a4/a3
         d2udx2 = -a4*(-PHI**2 + a3)*dexp(-PHI**2/(2*a3))/a3**2
                        
         Gcub_bc(4*i - 1) = ucub_bc(4*i - 1)*hcub_bc(4*i - 1) 
     .    -beta1/2d0*(d2udx2*hcub_bc(4*i - 1)**3 
     .    + 3*dhdx*dudx*hcub_bc(4*i - 1)**2)
         ...
         hcub_bc(4*i)  = EXPPHI1*a1 + a0
         ucub_bc(4*i)  = EXPPHI1*a4
         dhdx = -EXPPHI1*PHI*a1/a3
         dudx = -EXPPHI1*PHI*a4/a3
         d2udx2 = -a4*(-PHI**2 + a3)*dexp(-PHI**2/(2*a3))/a3**2
                        
         Gcub_bc(4*i) = ucub_bc(4*i)*hcub_bc(4*i) 
     .    -beta1/2d0*(d2udx2*hcub_bc(4*i)**3 
     .    + 3*dhdx*dudx*hcub_bc(4*i)**2)
...
c ====
     ...
     . ga,t,a0,a1,a2,a3,a4,a5,b1a6,b1a7,b2a6,b2a7,xbc,calctol,
     . nGhstCells,dx,dt, hcub,Gcub,ucub)
          
      integer nGhstCells,xbc_len,cubbc_len
      DOUBLE PRECISION habc(xbc_len),Gabc(xbc_len),xbc(xbc_len)
      DOUBLE PRECISION ga,t,theta,dx,dt,
     . a0,a1,a2,a3,a4,a5,b1a6,b1a7,b2a6,b2a7
     ...
     . nGhstCells,Gabc,dx,Gcub,calctol)
      ...
      !Calculate u
      call FEMforU(hcub,Gcub,ucub,xbc,t,a5,b1a6,b1a7,dx,xbc_len,
     .   cubbc_len, 3*xbc_len + 1,nGhstCells)
     
     
      !Euler Step
      call SingleEulerStep(xbc_len,cubbc_len,hcub,Gcub,ucub,ga,
     . t,a0,a1,a2,a3,a4,a5,b1a6,b1a7,b2a6,b2a7,xbc,nGhstCells,dt,dx,
     . habc,Gabc,hapbc,Gapbc)
     
      !update BC
      
      !update BC's, since EulerStep function, assumes its constant
      !BC's
      call ForcedSolution(xbc(1:nGhstCells),nGhstCells,dx,t+dt,
     . a0,a1,a2,a3,a4,a5,b1a6,b1a7,hcub(1:4*nGhstCells),
     . Gcub(1:4*nGhstCells),ucub(1:4*nGhstCells),4*nGhstCells)  
     ...
     .   dx,Gapbc(1:nGhstCells))   
      ...
      call ForcedSolution(xbc(xbc_len - nGhstCells + 1:xbc_len),
     . nGhstCells,dx,t+dt,a0,a1,a2,a3,a4,a5,b1a6,b1a7,
     . hcub(cubbc_len - 4*nGhstCells + 1 : cubbc_len),
     . Gcub(cubbc_len - 4*nGhstCells + 1 : cubbc_len),
     . ucub(cubbc_len - 4*nGhstCells + 1 : cubbc_len),
     . 4*nGhstCells)   
      call CubicToCellAverage(4*nGhstCells,nGhstCells,
     . hcub(cubbc_len - 4*nGhstCells + 1 : cubbc_len),
...
c ====== second time step 
     ...
     . nGhstCells,Gapbc,dx,Gcub,calctol)   
      ...
      !Calculate u
      call FEMforU(hcub,Gcub,ucub,xbc,t+dt,a5,b1a6,b1a7,dx,xbc_len,
     .   cubbc_len, 3*xbc_len + 1,nGhstCells)
     
      !Euler Step
      call SingleEulerStep(xbc_len,cubbc_len,hcub,Gcub,ucub,ga,
     . t + dt,a0,a1,a2,a3,a4,a5,b1a6,b1a7,b2a6,b2a7,
     . xbc,nGhstCells,dt,dx,
     . hapbc,Gapbc,happbc,Gappbc)
...
c ====
     ...
     . t,a0,a1,a2,a3,a4,a5,b1a6,b1a7,b2a6,b2a7,xbc,
     . nGhstCells,dt,dx,habc,Gabc,hapbc,Gapbc)
     
     
      integer nGhstCells,xbc_len,cubbc_len
      DOUBLE PRECISION dt,dx,ga,a0,a1,a2,a3,a4,a5,b1a6,b1a7,b2a6,b2a7,t
      DOUBLE PRECISION hcub(cubbc_len),Gcub(cubbc_len),ucub(cubbc_len)
     . ,hapbc(xbc_len),Gapbc(xbc_len),habc(xbc_len),Gabc(xbc_len),
     . xbc(xbc_len)
     
     
      DOUBLE PRECISION fih,fiG,foh,foG
     ...
     . b1a6,b1a7,b2a6,b2a7,dfluxGdx)      
         ...
         hapbc(i) = habc(i)  + dt*dhdt 
     .    + dt*( dfluxhdx  - (foh - fih)/dx ) 

         Gapbc(i) = Gabc(i)  + dt*dGdt 
...
c subroutine that cubics, calculates flux across 
     ...
     . - 27*ucub(4*(i+1)-1) 
     . + 9*ucub(4*(i+1)) )/ (2*dx**3)
      pup1b = (9*ucub(4*(i+1)-3) - 9*ucub(4*(i+1)-2) - 9*ucub(4*(i+1)-1) 
     . + 9*ucub(4*(i+1))  )/ (4*dx**2)
      pup1c = (ucub(4*(i+1)-3) - 27*ucub(4*(i+1)-2) + 27*ucub(4*(i+1)-1) 
     . - ucub(4*(i+1)) )/ (8*dx)
      pup1d = (-ucub(4*(i+1)-3) + 9*ucub(4*(i+1)-2) + 9*ucub(4*(i+1)-1) 
     . - ucub(4*(i+1)))/ 16.0

      php1a = (-9*hcub(4*(i+1)-3) + 27*hcub(4*(i+1)-2) 
     . - 27*hcub(4*(i+1)-1) 
     . + 9*hcub(4*(i+1)) )/ (2*dx**3)
      php1b = (9*hcub(4*(i+1)-3) - 9*hcub(4*(i+1)-2) - 9*hcub(4*(i+1)-1) 
     . + 9*hcub(4*(i+1))  )/ (4*dx**2)
      php1c = (hcub(4*(i+1)-3) - 27*hcub(4*(i+1)-2) + 27*hcub(4*(i+1)-1) 
     . - hcub(4*(i+1)) )/ (8*dx)
      php1d = (-hcub(4*(i+1)-3) + 9*hcub(4*(i+1)-2) + 9*hcub(4*(i+1)-1) 
     . - hcub(4*(i+1)))/ 16.0
...
c ===
      ...
      DOUBLE PRECISION x,t,a1,a2,a3,dhdt,EXPPHI1,PHI
      ...
      DOUBLE PRECISION t,a0,a1,a2,a3,a4,a5,b1a6,b1a7,dGdt, 
     . EXPPHI1,PHI,hi,ui,beta1
     ...
     . - 9*PHI**3*a1*a2*beta1*hi**2*ui*exp(-PHI**2/(2*a3))/(2*a3**3) 
     . - PHI**3*a2*a4*beta1*hi**3*exp(-PHI**2/(2*a3))/(2*a3**3) 
     . + PHI**2*a4*a5*b1a6*hi**3*exp(-PHI**2/(2*a3))/(2*a3**2) 
     . + 9*PHI*a1*a2*beta1*hi**2*ui*exp(-PHI**2/(2*a3))/(2*a3**2) 
     . + PHI*a2*hi*ui/a3 
     . + 3*PHI*a2*a4*beta1*hi**3*exp(-PHI**2/(2*a3))/(2*a3**2) 
     . - a4*a5*b1a6*hi**3*exp(-PHI**2/(2*a3))/(2*a3)
      end 
     ...
     . beta1,beta2, db1x, db2x
      ...
      d2hdx2 = -a1*(-PHI**2 + a3)*exp(-PHI**2/(2*a3))/a3**2
      d3hdx3 = PHI*a1*(-PHI**2 + 3*a3)*exp(-PHI**2/(2*a3))/a3**3
      d3udx3 = PHI*a4*(-PHI**2 + 3*a3)*exp(-PHI**2/(2*a3))/a3**3
      ...
      dfluxGdx = -2*d2udx2*dudx*hi**3*(beta1)
     .  - 3*dhdx*dudx**2*hi**2*(beta1) 
