     ...
     . habca(xbc_len),Gabca(xbc_len),uabca(xbc_len)
      ...
      call SmoothDB(xbc,xbc_len,dx,h0,h1,alpha,
     . hbccub_init,ubccub_init,Gbccub_init,cubic_len )
     ...
     . tlist,tlist_len,ExpWdir,ExpWdir_len)
      ...
      call SmoothDB(xbc,xbc_len,dx,h0,h1,alpha,
     . hbccub_fina,ubccub_fina,Gbccub_fina,cubic_len ) 
     ...
     . tolcalc,alpha,alphas,Cr,maxwavespeed,dt,tend,tstart
     
      DOUBLE PRECISION tlist(tlist_len)
      
      wdir = "../Results/Validation/Run/SDBRegSWWEalpha2/10s/"
      ...
      do expi = 8,8
         ...
         Cr = 0.5
         maxwavespeed = dsqrt(alphas*ga*max(h0,h1))
         dt  = (Cr / maxwavespeed) *dx




         call  SingleSDB(h0,h1,alpha,ga,beta1,beta2,
     . xstart,xbc_len,n_GhstCells,cubic_len,dx,tstart,tend,
     . dt,tolcalc,NormFile,AvgNormFile,EnergFile,tlist,tlist_len,
