...
c =====      
      subroutine SmoothDB(x,x_len,dx,hl,hr,alpha,h,u,G,cubbc_len)
      implicit none
      
      integer x_len,cubbc_len
      DOUBLE PRECISION x(x_len),h(cubbc_len),G(cubbc_len),u(cubbc_len)
      DOUBLE PRECISION hl,hr,alpha,dx,cxi
      ...
      do i=1,x_len 
         ...
         ! x_i-1/2
         cxi = x(i) - 0.5*dx
         h(4*i-3) = hl + (hr - hl)/2d0*(1d0 + dtanh(cxi/alpha))
         u(4*i-3) = 0d0
         G(4*i-3) = 0d0
         
         ! x_i-1/6
         cxi = x(i) - dx/6.0
         h(4*i-2) = hl + (hr - hl)/2d0*(1d0 + dtanh(cxi/alpha))
         u(4*i-2) = 0d0
         G(4*i-2) = 0d0

         ! x_i+1/6
         cxi = x(i) + dx/6.0
         h(4*i-1) = hl + (hr - hl)/2d0*(1d0 + dtanh(cxi/alpha))
         u(4*i-1) = 0d0
         G(4*i-1) = 0d0
         
         ! x_i-1/2
         cxi = x(i) + 0.5*dx
         h(4*i) = hl + (hr - hl)/2d0*(1d0 + dtanh(cxi/alpha))
         u(4*i) = 0d0
         G(4*i) = 0d0
