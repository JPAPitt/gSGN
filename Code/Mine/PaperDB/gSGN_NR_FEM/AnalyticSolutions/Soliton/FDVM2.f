...
c=====================================
     ...
     .   dx,Gabc)
      ...
      do while (currenttime  .LT. -1) !tend )   
...
c Cell Average To Cubic
     ...
     .    cellavgbc_q(i+2),cellavgbc_q(i+3),dx,pjtojp3a,
     .    pjtojp3b,pjtojp3c,pjtojp3d)  
         
         call P3jm1tojp2ABCD(cellavgbc_q(i-1),cellavgbc_q(i),
     .    cellavgbc_q(i+1),cellavgbc_q(i+2),dx,pjm1tojp2a,
     .    pjm1tojp2b,pjm1tojp2c,pjm1tojp2d)  

         call P3jm2tojp1ABCD(cellavgbc_q(i-2),cellavgbc_q(i-1),
     .    cellavgbc_q(i),cellavgbc_q(i+1),dx,pjm2tojp1a,
     .    pjm2tojp1b,pjm2tojp1c,pjm2tojp1d)           

         call P3jm3tojABCD(cellavgbc_q(i-3),cellavgbc_q(i-2),
     .    cellavgbc_q(i-1),cellavgbc_q(i),dx,pjm3toja,
     .    pjm3tojb,pjm3tojc,pjm3tojd)   

         call minmod(pjtojp3a ,pjm1tojp2a,
     .   pjm2tojp1a,pjm3toja,R3a )
     
         if (R3a .gt. 0) then
         call minmod(pjtojp3b ,pjm1tojp2b,
     .   pjm2tojp1b,pjm3toja,R3b )
         else
         
         end if
         
         
         call P2jtojp2A(cellavgbc_q(i),cellavgbc_q(i+1),
     .    cellavgbc_q(i+2),dx,P2A0)
         call P2jm1tojp1A(cellavgbc_q(i-1),cellavgbc_q(i),
     .    cellavgbc_q(i+1),dx,P2A1)
         call P2jm2tojA(cellavgbc_q(i-2),cellavgbc_q(i-1),
     .    cellavgbc_q(i),dx,P2A2)
         
         call P1jtojp1A(cellavgbc_q(i),cellavgbc_q(i+1),dx,P1A0)
         call P1jm1tojA(cellavgbc_q(i-1),cellavgbc_q(i),dx,P1A1)
         
         ! Constant Term
         call minmod(pjtojp3d -cellavgbc_q(i) ,
     .   pjm1tojp2d -cellavgbc_q(i),
     .   pjm2tojp1d -cellavgbc_q(i),pjm3tojd -cellavgbc_q(i),mmR3d  )
     
         R3d  = mmR3d   + cellavgbc_q(i)
         
         ! Linear Term
         call minmod(P1A0,P1A1,P1A0,P1A1,R1c)
         call minmod(pjtojp3c -R1c ,pjm1tojp2c -R1c,
     .   pjm2tojp1c -R1c,pjm3tojc -R1c,mmR3c  )
         R3c  = mmR3c   + R1c
         
         ! Quadratic Term
         call minmod(P2A0,P2A1,P2A2,P2A1,R2b)
         call minmod(pjtojp3b -R2b ,pjm1tojp2b -R2b,
     .   pjm2tojp1b -R2b,pjm3tojb -R2b,mmR3b  )
         R3b  = mmR3b   + R2b
     ...
     .   pjm2tojp1a,pjm3toja,R3a )
         ...
         R3b = 0
         R3c = 0
         R3d = 1 !cellavgbc_q(i) !pjm3tojd !pjm2tojp1d  !pjtojp3d !pjm1tojp2d 
...
c         R3c = R1c
c         R3d = cellavgbc_q(i)
c         print *, 'Start Cell', i
c         print *, 'P2', P2A0,P2A1,P2A2  
c         print *, 0d0,R2b,R1c, cellavgbc_q(i)
c        print *,'lim',R3a,R3b,R3c,R3d   
cc         print *,'P30',pjm3toja,pjm3tojb,pjm3tojc,pjm3tojd    
c         print *,'P31',pjm2tojp1a,pjm2tojp1b,pjm2tojp1c,pjm2tojp1d 
c         print *,'P32',pjm1tojp2a,pjm1tojp2b,pjm1tojp2c,pjm1tojp2d   
c         print *,'P33',pjtojp3a,pjtojp3b,pjtojp3c,pjtojp3d 
c         print *,'Add',0d0, R2b,R1c,cellavgbc_q(i) 
c         print *,'Max', max(pjm3toja,pjm2tojp1a,pjm1tojp2a,pjtojp3a),
c     .   max(pjm3tojb-R2b,pjm2tojp1b-R2b,pjm1tojp2b-R2b,pjtojp3b-R2b),    
c     .   max(pjm3tojc-R1c,pjm2tojp1c-R1c,pjm1tojp2c-R1c,pjtojp3c-R1c), 
c     .   max(pjm3tojd-cellavgbc_q(i),pjm2tojp1d-cellavgbc_q(i),
c     .   pjm1tojp2d-cellavgbc_q(i),pjtojp3d-cellavgbc_q(i))     
c         print *,'Min', min(pjm3toja,pjm2tojp1a,pjm1tojp2a,pjtojp3a),
c     .   min(pjm3tojb-R2b,pjm2tojp1b-R2b,pjm1tojp2b-R2b,pjtojp3b-R2b),    
c     .   min(pjm3tojc-R1c,pjm2tojp1c-R1c,pjm1tojp2c-R1c,pjtojp3c-R1c), 
c     .   min(pjm3tojd-cellavgbc_q(i),pjm2tojp1d-cellavgbc_q(i),
c     .   pjm1tojp2d-cellavgbc_q(i),pjtojp3d-cellavgbc_q(i))     
...
c         print *,cubicbc_q(4*i -3),cubicbc_q(4*i -2),cubicbc_q(4*i -1),
c     . cubicbc_q(4*i) 
c         print *, 'End Cell', i      
...
c ********************************************************************************
     ...
     . Gcub_bc(cubbc_len)
      ...
      DOUBLE PRECISION hjmh,hjph,Gjmh,Gjph
      ...
      do i = nGhstCell + 1,xbc_len - nGhstCell
         ...
         hjmh = 0.5*(hcub_bc(4*i -3) + hcub_bc(4*i -4))
         hjph = 0.5*(hcub_bc(4*i) + hcub_bc(4*i+1))

         Gjmh = 0.5*(Gcub_bc(4*i -3) + Gcub_bc(4*i -4))
         Gjph = 0.5*(Gcub_bc(4*i) + Gcub_bc(4*i+1))

        call  FEMelem(4,Gelem,uhelem,h3uxelem,beta1,dx,
     . hjmh,hcub_bc(4*i -2),hcub_bc(4*i -1),hjph,
     . Gjmh,Gcub_bc(4*i -2),Gcub_bc(4*i -1),Gjph)
...
c      print *, beta1,beta2,uir,uip1l,hir,hip1l,Gir,Gip1l
c      print *, hcub(4*i),hcub(4*i-1),hcub(4*i-2), hcub(4*i-3)
