...
c ===
     ...
     . paramfilename_len,ts_len,ts,Energys)
...
c      do j = 1,16
c      if (j .eq. 4) then
c         READ(1,*) str1, str1 , nGhstcell
c      else if (j .eq. 5) then
c         READ(1,*) str1, str1 , xbc_len
c      else if (j .eq. 6) then
c         READ(1,("(42X,E23.17E4)"))  dx
c      else if (j .eq. 11) then
c         READ(1,*) str1, str1 , ga
c      else if (j .eq. 15) then
c         READ(1,*) str1, str1 , beta1
c      else if (j .eq. 16) then
c         READ(1,*) str1, str1 , beta2
c      else
c         READ(1,*) str1,str1
c      end if
...
c get length of string without trailing whitespace      
     ...
     . (resultval .LE. n )  )
      ...
      INTEGER j,effeclenwdir
      DOUBLE PRECISION ts(ts_len),Energys(ts_len,4)
      
      CHARACTER(len =wdirlen) wdir
      
c      wdir = "/home/jp/Documents/Work/PostDoc/Projects/Steve/" //
c     .    "1DWaves/RegularisedSerre/Data/RAW/Models/" //
c     .    "gSGNForcedLimAll/ConstantBeta/SmoothDBInvestigation/" //
c     .    "REGSWWE/AspRat2to1/DBalpha0p1/Beta05/dx06/"
     ...
     .    "SWWESolver/AspRat2to1/DBalpha0p1/"     
      
      call LenTrim(wdir,wdirlen,effeclenwdir)
      
      call EnergyTimeSeries(wdir(1:effeclenwdir) ,effeclenwdir,
     . "Params.dat", 10,ts_len,ts,Energys)
     
     
      open(2, file = 'SWWESolver_DBa0p1_ETS.dat') 
      do j = 1,ts_len
         write(2,*) ts(j),Energys(j,1) ,Energys(j,2) ,Energys(j,3) ,
     .   Energys(j,4) 
      end do
