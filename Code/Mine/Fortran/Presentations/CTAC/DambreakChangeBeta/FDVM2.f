...
c=====================================
     ...
     . theta,dx,dt,n_GhstCells,xbc_len,
...
c      do while (currenttime  .LT. currenttime -1 )   
      
         if ((currenttime .LT. 50.0)) then
            beta1 = beta1list(1)
            beta2 = beta2list(1)
            
         else if ((currenttime .GE. 50.0) .AND. 
     .      (currenttime .LT. 100.0)) then
            beta1 = beta1list(2)
            beta2 = beta2list(2)
         
         else if ((currenttime .GE. 100.0) .AND. 
     .      (currenttime .LT. 150.0)) then
            beta1 = beta1list(3)
            beta2 = beta2list(3)
         else 
            beta1 = beta1list(4)
     ...
     .      .OR. (filecount .EQ. 1 ))  then
            ...
            write (strct,'(I8)') filecount
            open(8, file = ExpWdir// strct //'.dat') 
            do i = 1,xbc_len
               write(8,*) currenttime,beta1,beta2,xbc(i),hbc_fin(i),
     .            Gbc_fin(i),ubc_fin(i)
            end do
            close(8)
