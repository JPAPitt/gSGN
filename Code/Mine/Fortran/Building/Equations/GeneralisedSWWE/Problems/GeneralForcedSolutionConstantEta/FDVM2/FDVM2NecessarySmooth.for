...
c=====================================
      subroutine Solve(x_start,x_end,x_len, n_GhstCells,tstart,tend,eps,
     . dtodx,theta,ga,a0,a1,a2,a3,a4,FilePre,norm,C1EngMeas,dx)
      implicit none
      
      
      real*8,intent(in) :: x_start,x_end,theta,
     . ga,a0,a1,a2,a3,a4,tstart,tend,dtodx,eps
     ...
     . uAEbc(x_len + 2*n_GhstCells)
      ...
      call Initloc(xbc,tstart,eps,a0,a1,a2,a3,a4,hbc,Gbc,ubc)
...
c      open(6, file = FilePre//'Flux.dat', status = 'unknown') 
      ct = tstart
      cti = 0
      do while (ct < tend ) 
      !do while (cti < 10) 
         call EvolveStepWrap(hbc,Gbc,ubc,n_GhstCells,eps,dx,dt,theta,
...
c      close(6)
      ...
      !get u at end
      call GetufromhG(hbc,Gbc,ubc,eps,dx,n_GhstCells)
      
      !analytic values for h,u,G at t = tend
      call Initloc(xbc,ct,eps,a0,a1,a2,a3,a4,hAEbc,GAEbc,uAEbc)
...
c ====================      
      subroutine Initloc(x,t,eps,a0,a1,a2,a3,a4,h,G,u)
      implicit none
      
      real*8,dimension(:),intent(in) :: x
      real*8,intent(in) :: t,eps,a0,a1,a2,a3,a4
      real*8,dimension(size(x)), intent(out)   :: h,G,u
                   
      real*8 PHI,EXPPHI1,dhdx,dudx,d2udx2
      ...
      do i=1,n 
         ...
         G(i) = h(i)*u(i) -eps*(d2udx2*h(i)**3 + 3*dhdx*dudx*h(i)**2)
      end do 
...
c ==================      
      subroutine GetufromhG(hbc,Gbc,ubc,eps,dx,n_GhstCells)
      implicit none
      
      real*8,dimension(:),intent(in) :: hbc,Gbc
      real*8,dimension(:), intent(inout)   :: ubc
      real*8,intent(in) :: dx,eps
...
c ********************************************************************************
      ...
      subroutine FluxStep(hbc,Gbc,ubc,n_GhstCells,eps,dt,dx,theta,
     . xbc,t,a0,a1,a2,a3,a4,hp,Gp)
      integer, intent(in) :: n_GhstCells
      real*8, intent(in) :: theta,dt,dx,eps,t,a0,a1,a2,a3,a4
...
c     centered approximation
     ...
     . /(2*dx**2)
      ...
      cdhip1 = ReconLinLimGrad(hbc(i),hbc(i+1),hbc(i+2),theta)
      cdGip1 = ReconLinLimGrad(Gbc(i),Gbc(i+1),Gbc(i+2),theta)
      ...
      ddhip1l = (5*hbc(i+1) - 13*hbc(i+2) + 11*hbc(i+3) - 3*hbc(i+4))
     . /(2*dx**2)
     ...
     .      /(2*dx**2)
         ...
         !dhir = -EXPPHI1*PHI*a1/a3
         !dhip1l = dhir
         
         !duir = -EXPPHI1*PHI*a4/a3      
         !duip1l = duir
         
         !dduir = -a4*(-PHI**2 + a3)*dexp(-PHI**2/(2*a3))/a3**2
         
         !Gir = hir*uir -eps*(dduir*hir**3 + 3*dhir*duir*hir**2)
         !Gip1l = Gir
         
         !ddhir = -a1*(-PHI**2 + a3)*EXPPHI1/a3**2
         !ddhir = ddhir*(1 + dx**2)
         !ddhip1l = ddhir
     ...
     .      - (ga/2d0)*eps*(hip1l**2)*dhip1l**2
         ...
         dhdt = Forcedht(xbc(i),t,eps,a0,a1,a2,a3,a4) 
         dGdt = ForcedGt(xbc(i),t,eps,a0,a1,a2,a3,a4) 
         
         dfluxhdx = Forcedfluxhx(xbc(i),t,eps,a0,a1,a2,a3,a4) 
         dfluxGdx = ForcedfluxGx(xbc(i),t,ga,eps,a0,a1,a2,a3,a4)         
...
c         write(6,*) 'hG t,x',t,xbc(i)
c         write(6,*) 'hG qir',hir,Gir,uir,duir,dhir, ddhir
c         write(6,*) 'hG qip1l',hip1l,Gip1l,uip1l,duip1l,
c     .    dhip1l, ddhip1l 
c         write(6,*) 'G Flux',foG,fiG,
c     .      (foG - fiG)/dx, dfluxGdx ,
c     .      dt*( dfluxGdx  - (foG - fiG)/dx )  
c         write(6,*) 'h Flux',foh,fih,
c     .      (foh - fih)/dx, dfluxhdx ,
c     .      dt*( dfluxhdx  - (foh - fih)/dx )  
c     
c         write(6,*) ' '
...
c      write(6,*) '*****************************'
      ...
      function Forcedht(x,t,eps,a0,a1,a2,a3,a4) result (dhdt)
      
      real*8, intent(in) :: x,t,eps,a0,a1,a2,a3,a4
      ...
      function ForcedGt(x,t,eps,a0,a1,a2,a3,a4) result (dGdt)
      ...
      dGdt = -6*EXPPHI1**2*PHI**3*a1**2*a2*eps*hi*ui/a3**3 
     . + EXPPHI1*PHI*a1*a2*ui/a3 
     . - 9*PHI**3*a1*a2*eps*hi**2*ui*EXPPHI1/a3**3 
     . - PHI**3*a2*a4*eps*hi**3*EXPPHI1/a3**3 
     . + 9*PHI*a1*a2*eps*hi**2*ui*EXPPHI1/a3**2 
     . + PHI*a2*hi*ui/a3 
     . + 3*PHI*a2*a4*eps*hi**3*EXPPHI1/a3**2
      end function ForcedGt
      
      function Forcedfluxhx(x,t,eps,a0,a1,a2,a3,a4) 
     . result (dfluxhdx)
      ...
      function ForcedfluxGx(x,t,ga,eps,a0,a1,a2,a3,a4) 
     . result (dfluxGdx)
      
      real*8, intent(in) :: x,t,ga,eps,a0,a1,a2,a3,a4
      ...
      dfluxGdx = 
     .   -2*dhdx*eps*hi*(d2hdx2*ga*hi + dhdx**2*ga/2 + 2*dudx**2*hi)
     . + dhdx*ga*hi
     . + dudx*(-eps*(d2udx2*hi**3 + 3*dhdx*dudx*hi**2) + hi*ui) 
     . - eps*hi**2*
     . (2*d2hdx2*dhdx*ga + 4*d2udx2*dudx*hi 
     . + d3hdx3*ga*hi + 2*dhdx*dudx**2) 
     .
     . + ui*(dhdx*ui + dudx*hi 
     . - eps*(3*d2hdx2*dudx*hi**2 + 6*d2udx2*dhdx*hi**2 
     . + d3udx3*hi**3 + 6*dhdx**2*dudx*hi))
     
      end function ForcedfluxGx
      
      subroutine EvolveStepWrap(hbc,Gbc,ubc,n_GhstCells,eps,dx,dt,
     . theta,xbc,t,a0,a1,a2,a3,a4)
           
      real*8,dimension(:),intent(inout) :: hbc,Gbc,ubc
      real*8, dimension(:),intent(in) :: xbc
      real*8, intent(in) :: dx,dt,theta,eps,a0,a1,a2,a3,a4,t
      ...
      call FluxStep(hbc,Gbc,ubc,n_GhstCells,eps,dt,dx,theta,xbc
     . ,t,a0,a1,a2,a3,a4,hp,Gp)
    
      !BC's
      call Initloc(xbc(:n_GhstCells),
     .   t + dt,eps,a0,a1,a2,a3,a4,hpbc(:n_GhstCells),
     ...
     .   ubc(nbc - n_GhstCells + 1:nbc))
      ...
      call GetufromhG(hpbc,Gpbc,ubc,eps,dx,n_GhstCells)
      
      !Update cell averages
      call FluxStep(hpbc,Gpbc,ubc,n_GhstCells,eps,dt,dx,theta,xbc
     ...
     . ( Gbc(n_GhstCells + 1 : nbc - n_GhstCells ) + Gp(:))/2
      ...
      call Initloc(xbc(:n_GhstCells),
     .   t + dt,eps,a0,a1,a2,a3,a4,hbc(:n_GhstCells),
...
c ********************************************************************************
     ...
     . maxwavespeed,dtodx,eps,a0,a1,a2,a3,a4
      ...
      !eps controls dispersion (1/3 most like Serre, 0 is SWWE)
      eps = 1d0/3d0 !- some issue when eps != 0
     ...
     .   eps,dtodx,theta,ga,a0,a1,a2,a3,a4,
