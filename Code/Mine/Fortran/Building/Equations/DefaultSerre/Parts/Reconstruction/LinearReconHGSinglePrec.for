...
c=====================================
     ...
     . theta,Amp,Freq,FilePre,norm,dx)
      ...
      real,intent(in) :: x_start,x_end,theta,Amp,Freq
      integer, intent(in) :: x_len,n_PtsCell, n_GhstCells
      character (LEN=*), intent(in)   :: FilePre
      real, intent(out)   :: norm,dx
                   
      real ha(x_len),hc(x_len),xc(x_len)
      
      real xReconGhost(n_PtsCell*(x_len + 2*n_GhstCells)),
     . hR(n_PtsCell*(x_len + 2*n_GhstCells)),
     . hRec(n_PtsCell*(x_len + 2*n_GhstCells))
     
      real hbeg(n_PtsCell * n_GhstCells),
     ...
     . n_GhstCells)
      ...
      ha(:)  = (-Amp/Freq*(COS(Freq* (xc(:) + 0.5*dx)) 
     .           - COS(Freq* (xc(:) - 0.5*dx))))/dx

      ! define hc - cell node values
      hc(:)  = Amp*SIN(Freq*xc(:) )     
      
      ! define h - analytic value of h at reconstructed points
      hR(:)  = Amp*SIN(Freq*xReconGhost(:) ) 
...
c==============================
     ...
     . n_PtsCell, n_GhstCells) result (hR)
      ...
      real,intent(in) :: theta,dx
      real, dimension(:), intent(in)   :: h,x
      real,intent(in) :: hbeg(n_PtsCell*n_GhstCells),
     . hend(n_PtsCell*n_GhstCells)
      
      real, dimension( n_PtsCell*(SIZE(x)+ 2*n_GhstCells) ) :: hR
      
      integer i,x_len
      real fdh,mdh,bdh,cdh
      x_len = SIZE(x)
...
c ========================
     ...
     .   "/FortranTests/ReconSineSinglePrec/"
      ...
      do i=1,n
         ...
         call LinearRecon(0.0,100.0,100*(2**(i-1)),3,1,1.2,1.5,
