...
c=====================================
     ...
     . beta1,beta2,dtodx,theta,ga,a0,a1,a2,a3,a4,
     . FilePre,norm,dx)
     
      
      real*8,intent(in) :: x_start,x_end,theta,
     . ga,a0,a1,a2,a3,a4,tstart,tend,dtodx,beta1,beta2
     ...
     . uAEbc(x_len + 2*n_GhstCells)
      ...
      call Initloc(xbc,tstart,beta1,a0,a1,a2,a3,a4,hbc,Gbc,ubc)
...
c      open(6, file = FilePre//'Flux.dat', status = 'unknown') 
      ...
      do while (ct < tend ) 
      !do while (cti < 10) 
         call EvolveStepWrap(hbc,Gbc,ubc,n_GhstCells,beta1,beta2,
     .      dx,dt,theta,xbc,ct,a0,a1,a2,a3,a4)
         ct = ct + dt
         cti = cti + 1
...
c      close(6)
      ...
      call Initloc(xbc,ct,beta1,a0,a1,a2,a3,a4,hAEbc,GAEbc,uAEbc)
...
c ====================      
      subroutine Initloc(x,t,beta1,a0,a1,a2,a3,a4,h,G,u)
      implicit none
      
      real*8,dimension(:),intent(in) :: x
      real*8,intent(in) :: t,beta1,a0,a1,a2,a3,a4
...
c ********************************************************************************
      ...
      subroutine FluxStep(hbc,Gbc,ubc,n_GhstCells,beta1,beta2,
     . dt,dx,theta,xbc,t,a0,a1,a2,a3,a4,hp,Gp)
      integer, intent(in) :: n_GhstCells
      real*8, intent(in) :: theta,dt,dx,beta1,beta2,t,a0,a1,a2,a3,a4
...
c     centered approximation
     ...
     .      - beta2/2*ga*hip1l**2*(hip1l*ddhip1l + dhip1l**2/2)
         ...
         dGdt = ForcedGt(xbc(i),t,beta1,a0,a1,a2,a3,a4) 
         
         dfluxhdx = Forcedfluxhx(xbc(i),t,a0,a1,a2,a3,a4) 
         dfluxGdx = 
     .      ForcedfluxGx(xbc(i),t,ga,beta1,beta2,a0,a1,a2,a3,a4)        
...
c      write(6,*) '*****************************'
      ...
      function ForcedGt(x,t,beta1,a0,a1,a2,a3,a4) result (dGdt)
      
      real*8, intent(in) :: x,t,beta1,a0,a1,a2,a3,a4
      ...
      dGdt = -3.0*EXPPHI1**2*PHI**3*a1**2*a2*beta1*hi*ui/a3**3 
     . - 2*EXPPHI1**2*PHI**3*a1**2*a2*hi*ui/a3**3 
     . + EXPPHI1*PHI*a1*a2*ui/a3 
     . - 4.5*PHI**3*a1*a2*beta1*hi**2*ui*dexp(-PHI**2/(2*a3))/a3**3 
     . - 3*PHI**3*a1*a2*hi**2*ui*dexp(-PHI**2/(2*a3))/a3**3 
     . - 0.5*PHI**3*a2*a4*beta1*hi**3*dexp(-PHI**2/(2*a3))/a3**3 
     . - PHI**3*a2*a4*hi**3*dexp(-PHI**2/(2*a3))/(3*a3**3) 
     . + 4.5*PHI*a1*a2*beta1*hi**2*ui*dexp(-PHI**2/(2*a3))/a3**2 
     . + 3*PHI*a1*a2*hi**2*ui*dexp(-PHI**2/(2*a3))/a3**2 
     . + PHI*a2*hi*ui/a3 
     . + 1.5*PHI*a2*a4*beta1*hi**3*dexp(-PHI**2/(2*a3))/a3**2 
     ...
     . result (dfluxhdx)
      ...
      function ForcedfluxGx(x,t,ga,beta1,beta2,a0,a1,a2,a3,a4) 
     . result (dfluxGdx)
      
      real*8, intent(in) :: x,t,ga,beta1,beta2,a0,a1,a2,a3,a4
      ...
      dfluxGdx = -beta2*dhdx*ga*hi*(d2hdx2*hi + dhdx**2/2) 
     . - beta2*ga*hi**2*(2*d2hdx2*dhdx + d3hdx3*hi)/2 
     . - 2*d2udx2*dudx*hi**3*(beta1 + 2d0/3d0) 
     . - 3*dhdx*dudx**2*hi**2*(beta1 + 2d0/3d0) 
     . + dhdx*ga*hi 
     . + dudx*(hi*ui - (0.5*beta1 + 1d0/3d0)*
     .    (d2udx2*hi**3 + 3*dhdx*dudx*hi**2)) 
     . + ui*(dhdx*ui + dudx*hi + (-0.5*beta1 - 1d0/3d0)*
     .    (3*d2hdx2*dudx*hi**2 + 6*d2udx2*dhdx*hi**2 
     .       + d3udx3*hi**3 + 6*dhdx**2*dudx*hi))
     
      end function ForcedfluxGx
      
      subroutine EvolveStepWrap(hbc,Gbc,ubc,n_GhstCells,beta1,beta2,
     . dx,dt,theta,xbc,t,a0,a1,a2,a3,a4)
           
      real*8,dimension(:),intent(inout) :: hbc,Gbc,ubc
      real*8, dimension(:),intent(in) :: xbc
      real*8, intent(in) :: dx,dt,theta,beta1,beta2,a0,a1,a2,a3,a4,t
      ...
      call FluxStep(hbc,Gbc,ubc,n_GhstCells,beta1,beta2,
     . dt,dx,theta,xbc,t,a0,a1,a2,a3,a4,hp,Gp)
     
    
      !BC's
      call Initloc(xbc(:n_GhstCells),
     .   t + dt,beta1,a0,a1,a2,a3,a4,hpbc(:n_GhstCells),
     ...
     .   ubc(nbc - n_GhstCells + 1:nbc))
      ...
      !Update cell averages
      call FluxStep(hpbc,Gpbc,ubc,n_GhstCells,beta1,beta2,
     . dt,dx,theta,xbc,t + dt,a0,a1,a2,a3,a4,hp,Gp)
     ...
     . ( Gbc(n_GhstCells + 1 : nbc - n_GhstCells ) + Gp(:))/2
      ...
      call Initloc(xbc(:n_GhstCells),
     .   t + dt,beta1,a0,a1,a2,a3,a4,hbc(:n_GhstCells),
...
c ********************************************************************************
     ...
     .   "/Models/gSGN/Forced/beta11beta21/"
     ...
     . maxwavespeed,dtodx,beta1,beta2,a0,a1,a2,a3,a4
      ...
      !beta1,beta2 controls dispersion properties, regularisation properties
      beta1 = 1d0
      beta2 = 1d0
