...
c *******************************************
     ...
     . dtsep,ExpWdir,expwdirlen)
      ...
      DOUBLE PRECISION tstart,tend,ga,theta,dx,dt,
     . currenttime
...
c ==================      
     ...
     . RHS(xbc_len)
      ...
      DOUBLE PRECISION ht1,ht2,dhc,a0,a1
      ...
      do i=n_GhstCells+1,xbc_len - n_GhstCells 
         ...
         dhc =dabs(hbc(i+1) - hbc(i-1))/(2*dx)
         
         if (dhc .LE. a0) then
            beta1 = 0d0
         else if (( dhc .GT. a0)  .AND. (dhc .LE. a1)) then
            beta1 = (10)*(dhc - a0)/(a1 - a0)
         else 
            beta1 = 10d0
         end if 
...
c ====
     ...
     . hppbc(xbc_len),Gppbc(xbc_len)
      ...
      call SingleEulerStep(xbc_len,hbc,Gbc,ubc,ga
     . ,theta,n_GhstCells,dt,dx,hpbc,Gpbc)
      ...
      call SingleEulerStep(xbc_len,hpbc,Gpbc,ubc,ga
...
c ====
     ...
     .      Gbc(xbc_len - n_GhstCells + i)
      ...
      call Fluxxiph(xbc_len,hbc,Gbc,ubc,ga,theta,dx,i,
     . foh,foG,cdhi,cdGi)
...
c across cell i +1 (which will be cell i as the method moves to the next cell)    
      subroutine Fluxxiph(xbc_len,hbc,Gbc,ubc,ga,theta,dx,i,
     . foh,foG,cdhi,cdGi)
     ...
     . dhir,ddhir,dhip1l,ddhip1l, alpha,dhc,a0,a1
