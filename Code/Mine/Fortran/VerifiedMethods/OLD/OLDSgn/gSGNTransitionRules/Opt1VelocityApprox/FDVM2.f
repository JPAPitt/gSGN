...
c ==================      
     ...
     . RHS(xbc_len)
      ...
      DOUBLE PRECISION ht1,ht2,Gohim1,Gohi,Gohip1,dGoh,a0
      ...
      do i=n_GhstCells+1,xbc_len - n_GhstCells 
         ...
         a0 = 10d0
         Gohim1= Gbc(i-1)/hbc(i-1)
         Gohi = Gbc(i)/hbc(i)
         Gohip1 = Gbc(i+1)/hbc(i+1) 
         dGoh =dabs(Gohip1 - Gohim1)
         
         if (dGoh .LE. a0*dabs(Gohi)) then
            beta1 = -2d0/3d0
         else 
            beta1 = 0d0
         end if 
...
c across cell i +1 (which will be cell i as the method moves to the next cell)    
     ...
     . dhir,ddhir,dhip1l,ddhip1l, alpha,Gohim1,Gohi,Gohip1,dGoh,a0
