...
c=====================================
     ...
     . currenttime 
      DOUBLE PRECISION hbc_init(xbc_len),Gbc_init(xbc_len),
...
c *******************************************
     ...
     . ga,b0,b1,t0,t1,beta2,theta,dx,dt,n_GhstCells,xbc,xbc_len,
     . hbc_init,Gbc_init,ubc_init,
     ...
     . dtsep,ExpWdir,expwdirlen)
      ...
      DOUBLE PRECISION tstart,tend,ga,b0,b1,t0,t1,beta2,theta,dx,dt,
     . currenttime
     ...
     . Gbc_fin(xbc_len), ubc_fin(xbc_len)
      ...
      IF (currenttime .LE. t0) THEN
         beta1 = b0 
      ELSE IF ((currenttime .GT. t0) 
     . .AND. (currenttime .LT. t1)) THEN
         beta1 = ((b1 - b0)/ (t1 - t0))*(currenttime - t0) + b0
      ELSE
         beta1 = b1
      END IF
     ...
     .      .OR. (currenttime + dt  .GE. tend ))  then
            ...
            do i = 1,xbc_len
               write(5,*) currenttime,beta1,xbc(i),hbc_fin(i),
     .            Gbc_fin(i),ubc_fin(i)
            end do
            close(5)
