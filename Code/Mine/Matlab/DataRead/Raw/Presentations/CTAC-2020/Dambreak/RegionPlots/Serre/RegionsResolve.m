...
wdir = "AspRat2to1/DBalpha0p1/Beta00";
dxdir = '/dx06/';
...
resstart = 40;
resend = 60;
resstarti = round(((linestart - scenbeg)/dx));
resendi = round(((lineend - scenbeg)/dx));
ressep = 3;
...
t = End(1,1);
x = [End(linestarti:linesep:resstarti,2);End(resstarti:ressep:resendi,2);End(resendi:linesep:lineendi,2)  ];
h = [End(linestarti:linesep:resstarti,3);End(resstarti:ressep:resendi,3);End(resendi:linesep:lineendi,3)  ];
G = [End(linestarti:linesep:resstarti,4);End(resstarti:ressep:resendi,4);End(resendi:linesep:lineendi,4)  ];
u = [End(linestarti:linesep:resstarti,5);End(resstarti:ressep:resendi,5);End(resendi:linesep:lineendi,5)  ];
...
% fill(x2, inBetween, 'g','facealpha',.0,'LineStyle','--');
hold on
...
x2 = [[Region1ub Region1ub],fliplr([Region5ub Region5ub])];
inBetween = [[ymin ymax], fliplr([ymin ymax])];
